...
const basequery =
'WITH QUERY1 AS(SELECT pickuptime, TRIPID AS tripid FROM CHASTAIN.TRIP),';

async function find() {
  let query = basequery;
  const binds = {};
  query += "SNOWQ AS(SELECT COUNT(QUERY1.TRIPID) AS snowtrips, weather.time AS time FROM CHASTAIN.WEATHER, QUERY1 WHERE Condition = 'Snow' AND  extract(hour from QUERY1.pickuptime) = extract(hour from weather.time) AND extract(DAY from QUERY1.pickuptime) = extract(DAY from weather.time) AND  extract(MONTH from QUERY1.pickuptime) = extract(MONTH from weather.time) GROUP BY Condition, weather.time),";
  query += "CLEARQ AS(SELECT COUNT(QUERY1.TRIPID) AS cleartrips, weather.time AS time FROM CHASTAIN.WEATHER, QUERY1 WHERE Condition IN ('Clear','Clouds')	 AND  extract(hour from QUERY1.pickuptime) = extract(hour from weather.time) AND extract(DAY from QUERY1.pickuptime) = extract(DAY from weather.time) AND  extract(MONTH from QUERY1.pickuptime) = extract(MONTH from weather.time) GROUP BY Condition, weather.time),";
  query += "RAINQ AS(SELECT COUNT(QUERY1.TRIPID) AS raintrips, weather.time AS time FROM CHASTAIN.WEATHER, QUERY1 WHERE Condition IN ('Rain', 'Thunderstorm', 'Drizzle', 'Squall') AND  extract(hour from QUERY1.pickuptime) = extract(hour from weather.time) AND extract(DAY from QUERY1.pickuptime) = extract(DAY from weather.time) AND  extract(MONTH from QUERY1.pickuptime) = extract(MONTH from weather.time) GROUP BY Condition, weather.time),";
